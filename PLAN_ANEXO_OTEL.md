...
| 10 | Ubicación de los spans manuales | **B + D · Cola de Operaciones y listener asincrónico** | Viven en endpoints distintos, así que sus spans nunca coinciden en una traza: la redundancia es imposible por construcción. B tiene el trabajo real y la jerarquía ya existe en el código; D es el único lugar donde esta API cruza una frontera asincrónica, y sin llamadas salientes es el único sitio donde se puede mostrar continuidad de contexto. Se descartó A por ser redundante con D (misma traza del POST) y por ser un método de cuatro líneas. C no era una ubicación alternativa sino el nivel más profundo de B, así que pasó a la Decisión 11; además corre también en `OrderService`, o sea que habría llenado de spans el listado de pedidos. |
| 11 | Granularidad de anidamiento | **B · Padre con hijos, salvo el scoring** | Tres niveles: HTTP → `ops.listUnresolvedQuestions` → `ops.resolveOrders` + `ops.scoreAndSort`. El reparto de tiempo quedó visible (15.7ms → 0.9 + 3.4). El scoring es **un** span con `questions.scored.count`, no N: con 11 preguntas sin resolver y un refresco automático cada 30s, un span por pregunta inflaría la traza sin decir nada más. |
| 12 | Atributos de dominio | **B · Identificadores + datos de negocio** | Score, clasificación, cantidad puntuada, canales activos. Sin PII: el comprador viaja embebido en `Order` y estaba a mano en los dos servicios instrumentados, así que la omisión es deliberada. Tampoco viaja el texto de la pregunta, que es entrada libre. Mismo corte que DECISIONS.md ya aplicaba a logs y avisos. |
| 13 | Dónde se marca el error | **B · En el `ApiExceptionHandler`** | Único punto por el que pasan las cuatro familias, incluidas las que nacen antes del controller (conversión de tipos, Bean Validation) y nunca llegan a un service. Verificado que sin esto un 409 produce una traza idéntica a la de un éxito, porque el handler captura todo y nada escapa al `DispatcherServlet`. |
| 14 | Tratamiento de los 4xx | **A · Convención de OTel, con `error.type`** | Los 4xx no marcan ERROR: el problema es del cliente y un 409 de regla de negocio es el sistema funcionando bien. Quedan identificables por el atributo `error.type` y contabilizados en las métricas por status code, así que el requisito de distinguir éxito de error se cumple sin apartarse de la convención. Se descartó C (agregar un camino que falle a propósito) por ser código que existe solo para la demo. |
| 15 | Cuándo registrar la excepción | **B · Solo las inesperadas** | El stacktrace aparece donde sirve para diagnosticar. Una excepción de negocio es flujo previsto: llenar las trazas de stacktraces de reglas que funcionaron es ruido. El criterio ya estaba escrito en el código: el handler tiene un método por tipo de excepción. |
| 16 | Métrica custom | **A + C · Preguntas creadas por clasificación, y notificaciones por canal y resultado** | Dos counters baratos y complementarios: uno cuenta lo que entra y con qué gravedad, el otro hace observable el objetivo 3 del challenge principal, que no aparecía en ninguna señal. El aislamiento de fallos entre canales pasa de ser una línea de log a un dato consultable. |
| 17 | Etiquetas de las métricas | **A · Solo dimensiones acotadas** | Clasificación (4 valores), canal (3) y resultado (2). Ninguna crece con el tráfico. Se descartó el vendedor: es un UUID, o sea un identificador opaco como label, que es lo que el enunciado marca como algo a evitar. |
| 18 | Generación de tráfico | **A · Script de curls versionado** | `otel/generar-trafico.sh`, con los escenarios exitosos, la notificación asincrónica y cinco errores. Reproducible con un comando. Se descartó la UI porque está diseñada para no ofrecer acciones que el backend rechace, así que no puede producir los escenarios de error, que son la mitad de lo que hay que demostrar. |
| 19 | Ubicación de la documentación | **A · Documento propio con link desde el README** | `OBSERVABILITY.md`. El README del challenge principal ya tenía 412 líneas y describe un arranque (JDK, Gradle, Node) que el anexo reemplaza por Docker: integrarlo habría obligado a reescribir partes de la entrega anterior. Se agregó un aviso destacado arriba del README para que haya un solo punto de entrada. |
| 20 | Repositorio de entrega | **B durante el desarrollo, mergeado a `main` en la entrega** | El anexo se desarrolló en `anexo-otel` para que el challenge principal quedara congelado y el trabajo se leyera como un diff limpio sobre esa base — pesó que terminó tocando código existente (`ApiExceptionHandler`, `build.gradle`, wrapper de Gradle). Al entregar se mergeó a `main` con `--no-ff`, para que haya un solo punto de entrada sin perder la delimitación: los commits del anexo siguen siendo un bloque identificable en el historial. |
